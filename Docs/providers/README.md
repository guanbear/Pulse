...
Only Claude Code and Codex set `keepsLocalTranscripts`, which gates the labelled money estimate. The “working right now” mark is separately gated by `supportsLocalActivity`: Kiro CLI/ACP session records and ZCode's native/ACP turn telemetry can prove lifecycle state without supplying spend-ledger token buckets. ZCode activity is attributed only when its configured endpoint identifies Zhipu or z.ai. **History is the wider `providesHistory`**: Z.ai and Zhipu answer it from the account's own statistics instead ([zai.md](zai.md)). Unsupported values are left out rather than shown as zeroes. OpenCode *does* keep sessions (`opencode stats`); they live in OpenCode’s own store, not the JSONL the ledger reads, so the flag is false today.
