...
An announced scheduled event is shown only through its window; it disappears once the expected time passes, even if the feed has not yet marked it confirmed. Otherwise a separate confirmation can age off and leave the old prediction showing again as a false new alert. An unscheduled announcement expires after 48 hours. A confirmed event remains visible for 24 hours. These bounds keep an old, never-confirmed post from becoming a permanent status badge.

On the rail, current news is a small orange unread dot beside Codex's percentage. It scales with the Small, Standard and Large panel sizes without adding another rail slot or covering the provider mark. When percentage labels are hidden, the dot falls back to the corner of the existing ring so top docking does not lose the signal. Keeping it off the ring whenever a label exists makes the announcement distinct from the usage ring, optional second-limit ring, and window-clock arc. Hover still opens the ordinary usage card; the reset event appears below a divider inside the same pointed bubble, including attribution and the original-post excerpt. The attribution links to the first source post, falling back to the feed page when no post is present. Its status is deliberately “announced” or “confirmed”, never “received”. The whole auxiliary feed can be disabled in Codex's Panel settings; disabling it clears the section and dot and stops polling the feed.
