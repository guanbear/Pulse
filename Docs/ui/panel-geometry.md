...
- Codex reset news is a **second card beside** the usage card, never its replacement. `Layout.size(for:)` always budgets `DetailCardLayout + ResetEventCardLayout` even when no event exists; the transparent surplus is what lets news appear without resizing the window. The pair is one overlay group, while the rail remains outside that group. On the right edge the event precedes usage so usage stays nearest the rail; on the left/top it follows usage. Top docking keeps usage first so the pointer's along-axis origin remains unchanged.
