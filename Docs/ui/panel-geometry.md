...
- Codex reset news is a **second card below** the usage card, never its replacement. `Layout.size(for:)` always budgets the two cards' combined height even when no event exists; the transparent surplus is what lets news appear without resizing the window. The pair is one overlay group, while the rail remains outside that group. Usage stays first and owns the only pointer. At a side edge the event card gets pointer-width clear padding on the rail side, aligning the two card bodies without pretending the event card has a tail.
