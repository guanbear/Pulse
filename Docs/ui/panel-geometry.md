...
- Codex reset news is an **optional section inside** the usage card, never its replacement. A quiet divider separates third-party news from provider-reported usage, while one surface and one pointer keep the selected account unambiguous. `Layout.size(for:)` always budgets the combined content height even when no event exists; the transparent surplus is what lets news appear without resizing the window.
