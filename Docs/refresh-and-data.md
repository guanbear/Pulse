...
“Working” is not “written to recently.” The supported agents state the answer in the **tail** of their local lifecycle records. ZCode's native TUI and `zcode-acp` app-server use the same turn events; overlapping turn ids are reconciled independently. Kiro's Prompt, ToolResults and AssistantMessage records distinguish model work, tool work and a finished reply. Rules of thumb (detail and historical measurements: [providers/README.md](providers/README.md) and [decisions/reported-figures.md](decisions/reported-figures.md)):
